...
export const RoleSwitcher = React.forwardRef<HTMLDivElement, RoleSwitcherProps>(
  ({ currentView, onViewChange, className }, ref) => {
    ...
    if (loading) {
      return <div ref={ref} className={cn("h-9 w-32 bg-muted animate-pulse rounded-lg", className)} />;
    }
    ...
    if (!hasBothRoles) {
      return (
        <div
          ref={ref}
          className={cn("flex items-center gap-2 px-3 py-2 rounded-lg bg-muted/50", className)}
        >
          {currentView === "host" ? (
            <>
